...
const SubscriptionStep2 = ({selectedPackage}: Props) => {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            navigate('/thong-tin-ca-nhan/giao-dich'); // Điều hướng về trang lịch sử giao dịch
        } catch (error) {
            console.error(error);
            ...
            <div className="mb-4">
                {!isAuthenticated ? (
                    <div className="text-center py-4 border rounded bg-light">
                        ...
                        <p className="text-muted mb-4 small">Bạn cần đăng nhập để hệ thống ghi nhận gói cước vào tài
                            khoản.</p>
                        <div>
                    ...
                    <div className="alert alert-light border d-flex justify-content-between align-items-center p-3">
                        <div className="d-flex align-items-center">
                            ...
                            <div className="lh-1">
                                ...
                                <div
                                    className="fw-bold text-dark">{user?.emailOrPhone || user?.displayName || 'Thành viên'}</div>
                            </div>
                        </div>
                        <Button variant="link" className="text-decoration-none text-danger p-0 small"
            ...
            <div className="border rounded p-3 mb-4 bg-light bg-opacity-50">
                <div className="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <div
                            className="small text-secondary">{selectedPackage.durationDays > 0 ? `${selectedPackage.durationDays} ngày` : 'Theo kỳ'}</div>
                    </div>
            ...
            <div style={{opacity: isAuthenticated ? 1 : 0.6, pointerEvents: isAuthenticated ? 'auto' : 'none'}}>
                                ...
                                >
                                    <Card.Body className="d-flex align-items-center p-3">
                                        <div className="me-3 icon-wrapper">
                                            <img src={pm.icon} alt={pm.name} className="img-fluid"
                                                 style={{maxHeight: '100%', maxWidth: '100%', objectFit: 'contain'}}/>
                                        </div>
                                        <div className="flex-grow-1">
                                            <div
                                                className={`fw-bold ${isSelected ? 'text-primary' : 'text-dark'}`}>{pm.name}</div>
                ...
                <div className="form-check mb-4">
                    ...
                    <label className="form-check-label user-select-none small text-secondary" htmlFor="terms">
                        Tôi đồng ý với <a href="#" className="text-decoration-underline text-dark fw-bold">Quy định
                        thanh toán</a> và <a href="#" className="text-decoration-underline text-dark fw-bold">Chính sách
                        bảo mật</a> của Người Lao Động.
                    </label>
                </div>
                ...
                <div className="text-center">
                    ...
                    >
                        {isProcessing ? (
                            <span><span className="spinner-border spinner-border-sm me-2" role="status"
                                        aria-hidden="true"></span>Đang xử lý...</span>
                        ) : (
